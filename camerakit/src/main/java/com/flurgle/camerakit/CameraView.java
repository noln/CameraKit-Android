...
public class CameraView extends FrameLayout {
  ...
  public CameraView(@NonNull Context context, @Nullable AttributeSet attrs) {
    ...
    if (attrs != null) {
      ...
      try {
        ...
        mAdjustViewBounds = a
            .getBoolean(R.styleable.CameraView_android_adjustViewBounds, CameraKit.Defaults.DEFAULT_ADJUST_VIEW_BOUNDS);
      } finally {
        a.recycle();
      }
    }
  ...
  // TODO - Fix once library supports Camera2
  public Camera getCamera1Instance() throws InvalidObjectException {
    if (this.mCameraImpl instanceof Camera1) {
      return ((Camera1) mCameraImpl).getCamera();
    }
    else {
      throw new InvalidObjectException("!!! INVALID CAMERA IMPLEMENTATION !!!");
    }
  }
  ...
  private class CameraListenerMiddleWare extends CameraListener {
    ...
    public void onPictureTaken(byte[] jpeg) {
      ...
      if (mCropOutput) {
        int width = mMethod == METHOD_STANDARD ? mCameraImpl.getCaptureResolution().getWidth()
            : mCameraImpl.getPreviewResolution().getWidth();
        int height = mMethod == METHOD_STANDARD ? mCameraImpl.getCaptureResolution().getHeight()
            : mCameraImpl.getPreviewResolution().getHeight();
